...
type AdminContractorsPageProps = {
  params: Promise<{ locale: string }>;
};

export default async function AdminContractorsPage({
  params,
}: AdminContractorsPageProps) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl px-8 py-20">
      <h1 className="font-[var(--font-display)] text-5xl text-foreground">
        {t(locale, "admin.contractors.title")}
      </h1>
      <p className="mt-4 text-lg text-muted">
        {t(locale, "admin.contractors.subtitle")}
      </p>
      <section className="mt-8 rounded-2xl border border-foreground/10 bg-white/70 p-6">
        ...
        <p className="mt-6 text-sm text-muted">
          {t(locale, "admin.contractors.placeholder")}
        </p>
      </section>
      <div className="mt-8">
        <Link
        ...
        >
          {t(locale, "admin.contractors.back")}
        </Link>
      </div>
    </main>
  );
